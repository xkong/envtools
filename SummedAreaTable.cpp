...
void SummedAreaTable::createLum(float *rgb, const uint width, const uint height,
  ...
  for (uint y = 0; y < height; ++y) {
    ...
    // the latitude-longitude format overrepresents the area of regions near the
    // poles. To compensate for this, the pixels of the probe image should first
    // be scaled by cosφ. (φ == 0 at middle height of image input)
...
#define ENHANCE_PRECISION 1
  ...
  for (uint y = 0; y < _height; ++y) {
    for (uint x = 0; x < _width; ++x) {
      const uint i = y * width + x;
      _sat2[i] =
          I(x, y) * I(x, y) + I2(x - 1, y) + I2(x, y - 1) - I2(x - 1, y - 1);
    }
  }
  // integral 3
  for (uint y = 0; y < _height; ++y) {
    for (uint x = 0; x < _width; ++x) {
      const uint i = y * width + x;
      _sat3[i] = I(x, y) * I(x, y) * I(x, y) + I3(x - 1, y) + I3(x, y - 1) -
                 I3(x - 1, y - 1);
    }
  }
  // integral 4
