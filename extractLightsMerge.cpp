...
uint mergeLights(LightVector &lights, LightVector &newLights, const uint width,
  ...
  for (LightVector::iterator lightIt = lights.begin(); lightIt != lights.end();
       ++lightIt) {
    ...
    do {
      ...
      for (LightVector::iterator l = lights.begin(); l != lights.end(); ++l) {
        ...
        bool intersect2D = !(l->_y > y2 || l->_y + l->_h < y1 || l->_x > x2 ||
                             l->_x + l->_w < x1);
        ...
        /*
          ...
          l->_x-border > width + w || l->_x+l->_w+border < width); }else if( x+w
          == width ){
          //check right borders
          intersect2D = !(l->_y-border > y+h || l->_y+l->_h+border < y ||
          l->_x-border > w + (width - x) || l->_x+l->_w+border < (width - x));
          }
          }
        */
...
uint selectLights(LightVector &mergedLights, LightVector &newLights,
  ...
  for (LightVector::iterator lightIt = mergedLights.begin();
    ...
    if (lightIt->_mergedNum > 0) {
      ...
      for (LightVector::iterator l = lights.begin() + 1; l != lights.end();
           ++l) {
        l->_merged = false;
      }
      ...
      do {
        ...
        for (LightVector::iterator l = lights.begin() + 1; l != lights.end();
          ...
          if (intersect2D && intersectLightAgainstLights2D(
                                 lCurrent.childrenLights, *l, border)) {
...
uint mergeNearLights(LightVector &mergedLights, const double areaSizeMax,
  ...
  for (int i = 0; i < mergedLights.size(); ++i) {
    ...
    for (LightVector::iterator l = mergedLights.begin();
      ...
      const double newParentSizeW =
          std::max(lightCurrent->_x + lightCurrent->_w, (l->_x + l->_w)) - newX;
      if (lengthSizeMax < newParentSizeW)
        continue;
      const double newParentSizeH =
          std::max(lightCurrent->_y + lightCurrent->_h, (l->_y + l->_h)) - newY;
      if (lengthSizeMax < newParentSizeH)
        continue;
